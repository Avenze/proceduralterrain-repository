...
local APIURL = "http://94.254.64.114/projects/rosql/API/"
local APIFUNCS = {}
...
function _G.LoadRBXDatabase()
	...
	function API.Connect(_self, ApiKey)
		if (CurrentConnection ~= "") and (CurrentConnection ~= nil) then
			...
			local Response = JSONDecode(HttpService:PostAsync(APIURL.."connect.php", JSONEncode(Query), "TextPlain"))
	...
	function API.GetValue(_self, Key)
		if (CurrentConnection == "") or (CurrentConnection == nil) then
			...
			local Response = JSONDecode(HttpService:PostAsync(APIURL.."get.php", JSONEncode(Query), "TextPlain"))
	...
	function API.GetAllData(_self)
		if (CurrentConnection == "") or (CurrentConnection == nil) then
			...
			local Response = JSONDecode(HttpService:PostAsync(APIURL.."dump.php", JSONEncode(Query), "TextPlain"))
			if (Response.succ == true) then
				local New = {}
	...
	function API.SetValue(_self, Key, Value)
		if (CurrentConnection == "") or (CurrentConnection == nil) then
			...
			local Response = JSONDecode(HttpService:PostAsync(APIURL.."set.php", JSONEncode(Query), "TextPlain"))
			if (Response.succ == true) then
				return true
			else
				error("[ RBXDatabase ] "..Response.error, 2)
	...
	function API.UpdateValue(_self, Key, Func)
		if (CurrentConnection == "") or (CurrentConnection == nil) then
			...
			local Response = JSONDecode(HttpService:PostAsync(APIURL.."updateget.php", JSONEncode(Query), "TextPlain"))
			if (Response.succ == true) then
				...
				if tick()-StartTime < 23 then
					...
					Response = JSONDecode(HttpService:PostAsync(APIURL.."updateset.php", JSONEncode(Query), "TextPlain"))
					if (Response.succ == true) then
						return true
					else
						error("[ RBXDatabase ] "..Response.error, 2)
	...
	function API.DeleteValue(_self, Key)
		if (CurrentConnection == "") or (CurrentConnection == nil) then
			...
			local Response = JSONDecode(HttpService:PostAsync(APIURL.."delete.php", JSONEncode(Query), "TextPlain"))
			if (Response.succ == true) then
				return true
			else
				error("[ RBXDatabase ] "..Response.error, 2)
